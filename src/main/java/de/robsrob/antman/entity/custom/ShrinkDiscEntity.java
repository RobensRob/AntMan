...
public class ShrinkDiscEntity extends ThrowableItemProjectile {

    public ShrinkDiscEntity(EntityType<? extends ThrowableItemProjectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public ShrinkDiscEntity(Level pLevel) {
        super(ModEntities.THROWABLE_DISC.get(), pLevel);
    }

    public ShrinkDiscEntity(Level pLevel, LivingEntity livingEntity) {
    ...
    protected void onHitEntity(EntityHitResult pResult) {
        if(!this.level().isClientSide()) {
            ...
            if(target instanceof LivingEntity livingEntity) {
                ...
                livingEntity.setBoundingBox(livingEntity.getBoundingBox().inflate(.3));
